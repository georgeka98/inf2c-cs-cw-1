...
while_grid_not_end:
	...
	# end of grid found
	beq	$t7 $s2 horizontal_search
	blez	$t7 horizontal_search
	
	addi	$t6 $t6 1
	
	j	while_grid_not_end
...
h_next_dict_char:
	...
	beq	$t7 $s2 h_next_dict_char_inc
	blez	$t7 h_next_dict_char_inc
	
	j	h_while_potential_match_found
	
h_next_dict_char_inc:
...
d_next_char_for_potential_match:
	...
	subu	$t3 $t3 $s7			# max_grid_height - grid_init_row

	bge	$t8 $t3 d_next_grid_char_inc
	addu	$t3 $t3 $s7			# reset max_grid_height
	
	bne 	$s7 0 d_while_potential_match_found #  grid_init_row == 0
	blt	$s5 $t6 d_while_potential_match_found	# grid_init + match_d_row < max_grid_length

d_next_grid_char_inc:

	addu	$t3 $t3 $s7
	
	subu	$t8 $t8 $t3			# match_d_row = match_d_row - max_grid_height;
