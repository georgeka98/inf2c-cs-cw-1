...
void strfind()
{
  ...
  while(grid[max_grid_length] != '\n' && grid[max_grid_length] != '\0'){
    max_grid_length++;
  }
  ...
  while (grid[grid_idx] != '\0')
  {
    ...
    while(*word != '\n')
    {
      ...
      while(*match_word == *grid_char)
      {
        ...
        {
          match_clm++;
          match_clm = match_clm - max_grid_length;
          grid_char = &grid[match_clm];
        }
    ...
    while(row < max_grid_height)
    {
      ...
      while(*word != '\0')
      {
        ...
        while(*match_word == *grid_char)
        {
          ...
          {
            match_row = match_row - max_grid_height;
            grid_char = &grid[grid_clm + max_grid_length*match_row];
          }
  ...
  while (grid_init < max_grid_height*max_grid_length)
  {
    ...
    while(d_row < (max_grid_height - grid_init_row)  && ((d_row + grid_init < max_grid_length - 1 && grid_init_row == 0) || grid_init_row > 0)) // <---- please confirm)
    {
      word = dictionary + dictionary_idx[0]; //first char from dictionary ***
      ...
      while(*word != '\0')
      {
        ...
        while(*match_word == *grid_char)
        {
          ...
          if(match_d_row >= (max_grid_height - grid_init_row) || (grid_init + match_d_row >= max_grid_length && grid_init_row == 0))
          {
            match_d_row = (match_d_row - max_grid_height);

            grid_char = &grid[grid_init];
          }
